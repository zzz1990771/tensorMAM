...
mam_cv <- function(Y,X,ncv,K_index,r1_index,r2_index,r3_index,S,A,B,C,intercept,mu,degr,eps,max_step){
  n <- dim(Y)[1]
  q <- dim(Y)[2]
  p <- dim(X)[2]
  ...
  for(jj in 1:ncv){ # start CV
    ...
    for(K in K_index){
      ...
      for(r3 in r3_index){
        for(r2 in r2_index){
          for(r1 in r1_index){
            fit = Estimation(Ytrain,Ztrain,as.matrix(A[,1:r1]),as.matrix(B[1:K,1:r2]),as.matrix(C[,1:r3]),as.matrix(S[1:r3,1:(r1*r2)]),
                             as.numeric(intercept),mu,eps,max_step)
            Dnew  = fit$Dnew
