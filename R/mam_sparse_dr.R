...
mam_sparse_dr <- 
  function(Y,X,method="BIC",ncv=10,penalty="LASSO",K_index=NULL,r1_index=NULL,r2_index=NULL,r3_index=NULL,lambda=NULL,SABC=NULL,
           nlam=50,degr=3,lam_min=0.01,eps1=1e-4,maxstep1=20,eps2=1e-4,maxstep2=20,gamma=2,dfmax=NULL,alpha=1){
    n <- dim(Y)[1]
    q <- dim(Y)[2]
    p <- dim(X)[2]
    K1 <- 7
    ...
    if(method=="BIC") fit_dr = mam_sparse_bic(Y,X,K_index,r1_index,r2_index,r3_index,pen,lambda,A,B,C,S,
                                             nlam,degr,lam_min,eps1,maxstep1,eps2,maxstep2,gamma,dfmax,alpha,setlam)
    if(method=="CV") fit_dr = mam_sparse_cv(Y,X,ncv,K_index,r1_index,r2_index,r3_index,pen,lambda,A,B,C,S,
                                           nlam,degr,lam_min,eps1,maxstep1,eps2,maxstep2,gamma,dfmax,alpha,setlam)
