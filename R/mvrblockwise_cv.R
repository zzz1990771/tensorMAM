...
mvrblockwise_cv <- function(Y,X,Z,ncv,lambda,lens,opts,opts_pen){
  ...
  if(opts_pen$isPenColumn){
    ...
    fit_opt = EstMVR_blockwise(Y,X,Z,lambda_opt,lens,opts,opts_pen)
    ...
    fit_opt = EstMVR_glasso(Y,X,Z,lambda1,lens,opts,opts_pen)
    activeX = rbind(1,matrix(fit_opt$df[,nlam1],G))
