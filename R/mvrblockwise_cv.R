...
mvrblockwise_cv <- function(Y,X,Z,ncv,lambda,lens,opts,opts_pen){
  G <- opts$p
  q <- opts$q
  p = sum(lens)
  pz = opts$pz
  ...
  if(opts_pen$isPenColumn){
    ...
    for(jj in 1:ncv){ # start CV
      ...
      fit = EstMVR_blockwise(Ytrain,Xtrain,Ztrain,lambda,lens,opts,opts_pen)
      df = colSums(fit$df)
      if(pz){
        for(kk in 1:nlam){
    ...
    for(jj in 1:ncv){ # start CV
      ...
      fit = EstMVR_glasso(Ytrain,Xtrain,Ztrain,lambda,lens,opts,opts_pen)
      for(j in 1:q){
        if(pz){
          for(kk in 1:nlam){
  ...
  if(opts_pen$isPenColumn){
    fit_opt = EstMVR_blockwise(Y,X,Z,lambda,lens,opts,opts_pen)
    ...
    activeX = matrix(0,q,G)
    Bhat = matrix(0,p,q)
    likhd_opt = rep(0,q)
    bic_opt = matrix(0,q,nlam)
    if(pz) Chat = matrix(0,pz,q)
    else Chat = NULL
    
    fit_opt = EstMVR_glasso(Y,X,Z,lambda,lens,opts,opts_pen)
    for(j in 1:q){
      activeX[j,] = fit_opt$df[((j-1)*G+1):(j*G),selected[j]]
