...
mam_sparse_bic <- 
  ...
  for(K in K_index){
    ...
    for(r3 in r3_index){
      for(r2 in r2_index){
        for(r1 in r1_index){
          if(isPenColumn){
            fit = EstPenColumn(Y1,Z,as.matrix(A[,1:r1]),as.matrix(B[1:K,1:r2]),as.matrix(C[,1:r3]),as.matrix(S[1:r3,1:(r1*r2)]),
                               lambda,alpha,gamma,pen,dfmax,eps1,eps2,maxstep1,maxstep2) 
            df = r1*r2*r3+fit$df*r1+K*r2+q*r3-r1^2-r2^2-r3^2
          }
          else{
            fit = EstPenSingle(Y1,Z,as.matrix(A[,1:r1]),as.matrix(B[1:K,1:r2]),as.matrix(C[,1:r3]),as.matrix(S[1:r3,1:(r1*r2)]),
                               lambda,alpha,gamma,pen,dfmax,eps1,eps2,maxstep1,maxstep2) 
            df1 = NULL
            for(k in 1:nlam){
  ...
  if(intercept){
    mu = Ybar-fit_opt$Dnew%*%Zbar
    fit_opt$Dnew = cbind(as.vector(mu),fit_opt$Dnew)
  }
  else mu = rep(0,q)
